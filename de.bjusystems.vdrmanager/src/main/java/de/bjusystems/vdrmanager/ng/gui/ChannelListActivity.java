...
public class ChannelListActivity extends
	...
	public boolean onOptionsItemSelected(final MenuItem item) {
		...
		if (id == R.id.channels_groupby) {
									...
									public void onClick(DialogInterface dialog, int which) {
										...
										new VoidAsyncTask() {
											...
											protected Void doInBackground(Void... params) {
												if (reversed) {
													groupByReverse = !groupByReverse;
													Preferences.set(ChannelListActivity.this,
															P.CHANNELS_LAST_ORDER_REVERSE, groupByReverse);
												} else {
													Preferences.set(ChannelListActivity.this,
															P.CHANNELS_LAST_ORDER, groupBy);
												}
												return null;
											}
										}.execute();
			...
			return true;
		} else if (id == R.id.channels_recent_channels) {
			final String order = Preferences.get(ChannelListActivity.this, "gui_recent_channels_order", "most");
			List<RecenteChannel> rcs = null;

			if ("most".equals(order)) {
				rcs = DBAccess.get(ChannelListActivity.this)
						.getRecentChannelDAO()
						.loadByRecentUse(Preferences.get().getMaxRecentChannels());
			} else if ("last".equals(order)) {
				rcs = DBAccess.get(ChannelListActivity.this)
						.getRecentChannelDAO()
						.loadByLastAccess(Preferences.get().getMaxRecentChannels());
			} else {
				return true;
			}

			if (rcs.isEmpty() || Preferences.get().getMaxRecentChannels() <= 0) {
			...
			List<Channel> recentChannels = DBAccess.get(ChannelListActivity.this)
					.getRecentChannelDAO()
			...
			new AlertDialog.Builder(this)
					...
					.setAdapter(recentAdapter, new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
	...
	public boolean onContextItemSelected(final MenuItem item) {
		...
		if (type == ExpandableListView.PACKED_POSITION_TYPE_CHILD) {
			...
			if (id == R.id.channel_item_menu_stream) {
				// Show live stream
				Utils.stream(this, channel);
			} else if (id == R.id.channel_item_menu_switch) {
				Utils.switchTo(this, channel);
			} else {
				return false; // oder super.onContextItemSelected(item), falls nötig
			}

			return true;
		} else if (type == ExpandableListView.PACKED_POSITION_TYPE_GROUP) {
			final int groupPos = ExpandableListView.getPackedPositionGroup(info.packedPosition);
			// Gruppenlogik hier, falls nötig
			return true;
		}
